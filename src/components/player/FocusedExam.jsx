  ...
  return (
    <div style={{position:"relative",marginTop:8,marginBottom:8}}>
      ...
      <div style={{position:"relative",borderRadius:14,overflow:"hidden",background:"radial-gradient(60% 50% at 50% 38%,rgba(78,205,196,0.10),transparent 70%)",border:"1px solid rgba(255,255,255,0.06)",padding:"6px 0 2px",marginBottom:8}}>
        ...
        <div style={{position:"absolute",left:10,top:9,fontSize:10,fontWeight:700,color:"#4ECDC4",background:"rgba(78,205,196,0.12)",padding:"4px 9px",borderRadius:20}}>Tap a region to examine</div>
      </div>
      ...
      <div style={{fontSize:10,color:"#7f8694",fontWeight:600,marginTop:7,textAlign:"center"}}>{nDone + " of " + exams.length + " examined · exploring, not graded"}</div>
      ...
      <WhyModal open={!!whyTarget} onClose={function(){setWhyTarget(null);}} title={whyTarget?whyTarget.label:""} body={whyTarget?(whyTarget.why||"No additional explanation available for this finding."):""} accent={whyTarget&&whyTarget._abnormal?"#FF6B81":"#4ECDC4"} item={whyTarget?{id:signCanonicalId(whyTarget),kind:"sign",phaseIdx:phaseIdx,label:whyTarget.label,_slotRef:{kind:"sign",phaseIdx:phaseIdx,indexOrId:whyTarget.label}}:null}/>
    </div>
  );
}
